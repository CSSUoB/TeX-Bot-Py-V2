...
This is a [Discord bot](https://discord.com/developers/docs/topics/oauth2#bot-vs-user-accounts) used for managing a community group's [Discord](https://discord.com) [guild](TERMINOLOGY.md#guild-vs-server).
Featured in the [CSS Discord guild](https://cssbham.com/discord).

## Features

* Membership induction and role management workflows for [Discord members](TERMINOLOGY.md#user-vs-member-vs-guest).
* Moderation utilities such as strikes, archiving, and delete-all tooling.
* Automated reminders for introductions and opt-in roles.
* Statistics and reporting commands for guild activity.
* Utility commands for reminders, invite links, and message edits.

## Error Codes

[Users](TERMINOLOGY.md#user-vs-member-vs-guest) of TeX-Bot may encounter an error code when executing a [slash command](TERMINOLOGY.md#slash-commands) fails.
...
* `E1011` - The value for the [environment variable](https://wikipedia.org/wiki/Environment_variable) `DISCORD_GUILD_ID` is an [ID](https://discord.com/developers/docs/reference#snowflakes) that references a [Discord guild](TERMINOLOGY.md#guild-vs-server) that does not exist
