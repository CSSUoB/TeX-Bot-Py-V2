...
class CheckTokenCommandCog(TeXBotBaseCog):
    """Cog class that defines the /check_token command."""

    @discord.slash_command( # type: ignore[no-untyped-call, misc]
        name="check_token",
        description="Checks the provided token status.",
    )
    @CommandChecks.check_interaction_user_has_committee_role
    @CommandChecks.check_interaction_user_in_main_guild
    async def check_token(self, ctx: TeXBotApplicationContext) -> None:
        """
        Definition of the check token command.
        ...
        async with aiohttp.ClientSession(headers=request_headers, cookies=request_cookies) as http_session:  # noqa: E501, SIM117
            async with http_session.get("https://guildofstudents.com/profile") as http_response:  # noqa: E501
        ...
        if parsed_html is None or isinstance(parsed_html, bs4.NavigableString):
            logger.debug("No admin table was found, meaning the token provided does not have admin access to any societies.")  # noqa: E501
